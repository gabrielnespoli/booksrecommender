...
class ContentBased:
    ...
    def get_k_similar(self, query_isbn, w=[1,2,1]):
        ...
        # remove duplicate names (even if they have different ISBNs)
        s = set()
        sim_list = [item for item in sim_list[:2*self.k] if item[1] not in s and not s.add(item[1])]
    ...
    def get_similar_books(self):
        ...
        # remove duplicate names (even if they have different ISBNs)
        s = set()
        k_similars = [item for item in k_similars[:self.k] if item[1] not in s and not s.add(item[1])]
